...
(function (window) {
    ...
    function createImage(onload) {
        ...
        image.load = onload;
        // 这个load事件 在vue中没有生效 导致canvas绘制时的背景图片没有显示出来 
        image.src = getRoundImage()
        return image
    }
    function getRoundImage() {
        return imageSrcArr[getRoundNumber(1, 10)]
    }
    ...
    DragImage.prototype = {
        ...
        initImage: function () {
            setTimeout(() => {
                var self = this;
                self.draw()
                var image2 = document.getElementById('imgd');
                self.canCtx.drawImage(image2, 0, 0, self.width, self.height)
                self.bloCtx.drawImage(image2, 0, 0, self.width, self.height)
